...
filename = strcat('comsol_data/6_speaker_only_line.txt');
...
bp_power = sum(abs(bpnew).^2, 2);
bpnew = bpnew ./ sqrt(bp_power);


figure(11)
clf
plot(db(abs(diag(bpnew))))
saveas(gcf, 'figs/matlab_test.png')

bpnew = bpnew(31:151, 31:151);
...
pbaspect([1, 1, 1])
% saveas(gcf, 'figs/matlab_test.png')
print('-depsc', 'fig_results/beam_pattern_speaker_only_comsol.eps')
